        ...
        <div class="flex-1">
            ...
            <form v-if="editing" @submit.prevent="form.put(route('chirps.update', chirp.id), { onSuccess: () => editing = false })">
                <textarea 
                    ...
                    placeholder="Edit your chirp here..."
                ></textarea>
                <InputError :message="form.errors.message" class="mt-2" />
                <div class="space-x-2">
                    <PrimaryButton class="mt-4">Save</PrimaryButton>
                    <button class="mt-4 text-gray-600" @click="editing = false; form.reset(); form.clearErrors()">Cancel</button>
                </div>
            </form>

            <!-- Tampilan Chirp -->
            <div v-else class="mt-4 text-lg text-gray-900" v-html="chirp.message"></div>
            <!-- Tampilan Media -->
            <div v-if="chirp.media" class="mt-4">
                <img 
                    v-if="chirp.avatar" 
                    :src="chirp.avatar_url" 
                    alt="Chirp Avatar" 
                    class="max-w-full rounded-lg" 
                />
                <video 
                    v-else-if="chirp.media.type === 'video'" 
                    :src="chirp.media.url" 
                    controls 
                    class="max-w-full rounded-lg"
                >
